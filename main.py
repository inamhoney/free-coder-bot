    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if not result:
                    raise Exception("Model returned empty response — try again or use /model to switch")
                return result
    except asyncio.TimeoutError:
        raise Exception("Request timed out (model took too long). Try /model step for faster responses.")
